...
/* --------------OPTION SO_REUSEADDR------------------*/
	...
	int optval = 1;
	cout << "retour setsockotp(reuseaddr) : " << setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) << endl << endl;
		

/* --------------OPTION SO_RCVBUF------------------*/

	int maxBuffer = 150;
	cout << "retour setsockotp(rcvbuf) : " << setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &maxBuffer, sizeof(maxBuffer)) << endl << endl;

/* --------------OPTION SO_SNDLOWAT------------------*/

	cout << "retour setsockotp(sndlowat) : " << setsockopt(sock, SOL_SOCKET, SO_SNDLOWAT, &maxBuffer, sizeof(maxBuffer)) << endl << endl;
...
/* Verifie l'etat de l'option keepalive */
   ...
   cout <<"SO_KEEPALIVE is " << (optval ? "ON" : "OFF") << endl << endl;


   /* Rend l'option active */
   optval = 1;
   optlen = sizeof(optval);
   cout << "retour setsockotp(keepalive) : " << setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, &optval,optlen) <<  	    endl << endl;
...
int main(){
	...
	*/
	...
	cout << "Entree dans la boucle infinie" << endl;
	while(1){
	  ...
	  */
	  ...
	  if (newsockfd == -1) {
		cout << "Erreur accept" << endl;
		return (-1);
	  }
	  else
		cout << "Accept reussi" << endl;
	...
	cout << "Le fichier 'resultatRecuitSimule.txt', contenant les informations permettant le dessin du graphe et de sa solution via l'application bsplines, a ete genere." << endl << endl;
